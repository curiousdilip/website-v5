...
export default function Project({ project }) {
  if (!project) {
    return <div className="container container__padding-block">Project not found.</div>;
  }
          ...
          <div className="p-content">
            <div className="p-content__description">
              ...
              <motion.div
                ...
                animate="visible"
              >
                <motion.p className="p-content__description-info__job-role" variants={item}>
                  <span className="font-weight--semi-bold">Job Role:</span> {project.metadata?.job_role}
                </motion.p>
                <motion.p className="p-content__description-info__client" variants={item}>
                  <span className="font-weight--semi-bold">Client:</span> {project.metadata?.client}
                </motion.p>
                <motion.p className="p-content__description-info__year" variants={item}>
                  <span className="font-weight--semi-bold">Year:</span> {project.metadata?.year}
                </motion.p>
                <motion.p className="p-content__description-info__site" variants={item}>
                  <span className="font-weight--semi-bold">Website:   &nbsp;</span>
                  {project.metadata?.link ? (
                    <Link
                      href={project.metadata.link}
                      target="_blank"
            ...
            >
              {project.metadata?.technologies?.map(({ technology }, index) => (
                <motion.p key={index} className="p-content__technologies-tech" variants={item}>
                  {technology}
                </motion.p>
              ))}
            </motion.div>
...
export async function getStaticPaths() {
  try {
    ...
    const url = 'https://api.cosmicjs.com/v3/buckets/my-website-website/objects?pretty=true&query=%7B%22type%22:%22works%22%7D&limit=10&skip=0&read_key=n3jLniptSDz6sl6YWVXuEict2MsOxs3jN3sxxFOBg2vY8d0NIE&depth=1&props=slug,title,metadata,type';
